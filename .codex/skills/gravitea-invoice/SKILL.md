...
class Comprobante(TenantBoundModel):
    ...
    class Status(models.TextChoices):
        PENDING = "PENDING", "Pending"          # CbteNro reserved
        PROCESSING = "PROCESSING", "Processing"  # Sent to ARCA
        AUTHORIZED = "AUTHORIZED", "Authorized"  # CAE obtained
        REJECTED = "REJECTED", "Rejected"        # ARCA rejected
    ...
    # Amounts (Decimal 2 places, matching ARCA)
    imp_total = models.DecimalField(max_digits=17, decimal_places=2)
    imp_neto = models.DecimalField(max_digits=17, decimal_places=2)
    imp_iva = models.DecimalField(max_digits=17, decimal_places=2, default=Decimal("0"))
    imp_trib = models.DecimalField(max_digits=17, decimal_places=2, default=Decimal("0"))
    imp_op_ex = models.DecimalField(max_digits=17, decimal_places=2, default=Decimal("0"))
    imp_tot_conc = models.DecimalField(max_digits=17, decimal_places=2, default=Decimal("0"))
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            ...
            if existing and existing.status == self.Status.AUTHORIZED:
                raise ValueError(
...
class InvoiceService:
    ...
    4. Save comprobante as PENDING in DB
    5. Request CAE (FECAESolicitar)
    6. Update comprobante with CAE and status AUTHORIZED
    """
    ...
    def issue_comprobante(
        ...
        # Step 3: Save as PENDING (reserve the number)
        ...
        # Step 4: Request CAE
        try:
            comprobante.status = Comprobante.Status.PROCESSING
            comprobante.save()
            ...
            comprobante.status = Comprobante.Status.AUTHORIZED
            comprobante.arca_result = cae_result
            comprobante.save()

        except ARCAError as e:
            comprobante.status = Comprobante.Status.REJECTED
...
|   |-- Call FECAESolicitar
|   |-- Resultado = "A"? -> Store CAE + mark AUTHORIZED
|   +-- Resultado = "R"? -> Store observations + mark REJECTED
|
+-- After CAE obtained?
...
class TestComprobanteImmutability:
    def test_cannot_modify_authorized(self, comprobante_factory):
        ...
        comprobante = comprobante_factory(status=Comprobante.Status.AUTHORIZED)
